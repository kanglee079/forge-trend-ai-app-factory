...
export async function runFactoryE2E({ requireCodex = false, assertCodex = false, assertWebEvidence = false, payload = briefPayload() } = {}) {
  ...
  console.log(`Artifacts: PRD=${hasPrd} design=${hasDesign} source=${hasSource} build=${hasBuild} run_report=${hasRunReport}`);

  if (!hasPrd || !hasDesign || !hasSource || !dump.qa.length || !dump.policy.length || !hasRunReport) {
    throw new Error("Factory run reached terminal status but expected PRD/design/source/QA/policy/run-report outputs are missing.");
  }
  if (!hasBuild) {
